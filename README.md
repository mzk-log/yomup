...
- **マウスオーバーハイライト** — ホバー位置のテキストを読みやすい単位で**下線**表示（日英自動判定。块の目安は `constants.js` の `MAX_TEXT_LENGTH_FOR_HIGHLIGHT` / `MAX_WORDS_FOR_HIGHLIGHT`）。h1–h4 見出しは見出しテキスト幅のみ
- **ライン進行** — 読書速度に連動して下線が行順に伸長（→ トグルで ON/OFF）。ページクリックで一時停止／再開
- **読書速度** — 読むプ窓の `<select>` で変更（150 / 250 / 500 / 750 / 1000 字/分。英文は語/分換算）
- **文字数カウント** — ページ全体または選択範囲の文字数（日本文）／語数（英文）
- **読了時間の目安** — 上記読書速度設定に連動
- **ストップウォッチ／部分タイマー** — 読書時間の計測と、ハイライト確定後のカウントダウン（0 秒以降も継続）
...
- **Web 版と同等の支援** — 下線ハイライト、ライン進行、読書速度、ストップウォッチ、部分タイマー
...
- ハイライトがうまくできない箇所に対応する（難易度高。デグレが起きやすい）
    ...
    - 2026/6/20 青空文庫、developer.chrome 型 leaf `div`（3.2.0）
