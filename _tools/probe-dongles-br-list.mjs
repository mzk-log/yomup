/**
 * DG-1 調査 — dongles.jp 実績 <p> + <br> 行一覧が複数行まとめて光る
 * Usage:
 *   node _tools/probe-dongles-br-list.mjs
 *   node _tools/probe-dongles-br-list.mjs --live
 */
...
const USER_DATA = path.join(__dirname, '.pw-dongles-br-list');
...
const LIVE_URL = 'https://dongles.jp/enterpreneurship/';

const LINES = [
  '未来創造たまご塾古川中学校 (ナウエ様協賛)',
  '未来創造たまご塾啓晴高校 (久保田工務店様協賛)',
  '第3回高校生アントレプレナーシップ研修in岐阜大学（岐阜大学共同研究事業）',
  '第5回中高生アントレプレナーシップ研修in岐阜大学（名古屋大学共同研究事業）',
  '未来創造たまご塾 in かりや（刈谷市委託事業）'
];

const FIXTURE = `<!doctype html>
<html lang="ja"><head><meta charset="utf-8"><title>DG-1 br-list</title>
<style>
  body { font-family: "Yu Gothic", sans-serif; font-size: 16px; line-height: 1.8; max-width: 720px; margin: 40px; }
</style></head><body>
<div class="entry-content">
  <h4>2025年度</h4>
  <p class="wp-block-paragraph" id="achievements">${LINES.join('<br>')}<br>富山県起業家育成塾（公益財団法人富山県ひとづくり財団委託事業）</p>
  <p id="prose">これは通常の散文です。句点で文が分かれます。改行はありません。</p>
</div>
</body></html>`;
...
if (LIVE) {
  ...
  const fixturePath = path.join(os.tmpdir(), 'yomup-dg1-br-list.html');
  ...
  return page.evaluate(
    ({ sel, lineNeedles }) => {
      ...
      for (const needle of lineNeedles) {
        ...
        covered.push({ needle: needle.slice(0, 20), hit });
      }
      return {
        lit: segs.length > 0,
...
const facts = await page.evaluate(() => {
  const p = [...document.querySelectorAll('p')].find((el) =>
    (el.textContent || '').includes('未来創造たまご塾古川中学校')
  );
  ...
  return {
    ...
    lens: parts.slice(0, 6).map((t) => t.length),
    sum4: parts.slice(0, 4).reduce((a, t) => a + t.length, 0),
    sample: parts.slice(0, 4)
  };
});
console.log('facts:', JSON.stringify(facts, null, 2));

const pt = await locateSnippet(page, '古川中学校');
if (!pt) {
...
// scroll 後に座標を取り直す
const pt2 = (await locateSnippet(page, '古川中学校')) || pt;
console.log('hover point', pt2);
await dispatchMove(page, pt2.x, pt2.y);
const m = await measure(page, LINES.slice(0, 5));
